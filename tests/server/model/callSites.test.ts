...
import type { RoleDefinition } from '../../../src/server/model/charter.js'

const charter: readonly RoleDefinition[] = [
...
describe('callSites', () => {
  it('lists every participant, followed by the two operation call sites', () => {
    expect(callSites(charter).map((site) => site.site)).toEqual(['shape', 'story-editor', 'apply', 'capture'])
  })
